...
class augmentor(params):
	def __init__(self):
		...
		self.train_image_dimension=(2*self.batch_size*self.augmentation_count,self.height,self.width,3)
		self.train_gt_dimension=(2*self.batch_size*self.augmentation_count,self.height,self.width,self.num_classes)
		self.val_image_dimension=(2*self.batch_size,self.height,self.width,3)
		self.val_gt_dimension=(2*self.batch_size,self.height,self.width,self.num_classes)
		...
		if self.augmentation:
			...
			for i in range(self.augmentation_count):
				for j in range(self.batch_size):
					...
					augmented_data[self.batch_size*i+j+self.batch_size*self.augmentation_count,:,:,:]=augmented_data[self.batch_size*i+j,:,::-1,:]
					augmented_gt[self.batch_size*i+j+self.batch_size*self.augmentation_count,:,:,:]=gt[j,:,::-1,:]
					names_=names_+names
					names_=names_+names
